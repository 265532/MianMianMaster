...
import { mockJobPositions, mockSkillTree, mockJobMatchResults } from '../data/job.mock'
...
export function registerJobHandlers(mock: MockAdapter): void {
  mock.onPost('/jobs').reply((config) => {
    ...
    const newJob = { id: Date.now(), ...data, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
  ...
  mock.onGet(/\/jobs\/\d+\/match$/).reply((config) => {
    ...
    const result = mockJobMatchResults[jobId] || { job_id: jobId, match_score: 0, matched_skills: [], missing_skills: [] }
