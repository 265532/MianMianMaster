...
const getPathDetails = () => {
  return {
    ...
    description:
      pathName.value === "简历押题专项"
        ? "预测面试官可能会提问的问题，帮助您提前准备"
        : "模拟完整的面试流程，包括技术面、行为面等环节",
...
<template>
  <div class="flex flex-col gap-8 max-w-7xl mx-auto">
    <!-- 顶部导航 -->
    <div
      class="sticky top-0 z-10 bg-white shadow-sm border-b border-neutral-border"
    >
      <div class="px-4 py-4 flex items-center gap-4">
        <button
          class="p-2 hover:bg-neutral-bg rounded-full transition-colors"
          @click="goBack"
        >
    ...
    >
      <div class="p-6 bg-neutral-bg rounded-[24px]">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          ...
          <div class="p-4 bg-white rounded-xl text-center">
            <p class="text-lg font-black text-primary">
              {{ pathDetails.estimatedTime }}分钟
            </p>
            <p class="text-xs text-neutral-helper">预计时间</p>
          </div>
          <div class="p-4 bg-white rounded-xl text-center">
            <p class="text-lg font-black text-primary">
              {{ pathDetails.passRate }}%
            </p>
      ...
      <div v-if="isPracticeStarted" class="mt-8">
        ...
        <div class="space-y-2">
          ...
          <div class="h-2 bg-neutral-bg rounded-full overflow-hidden">
            <div
              class="h-full gradient-primary transition-all duration-300 ease-out"
      ...
      <div class="flex gap-4 justify-end pt-8 border-t border-neutral-border">
        ...
        <button
          ...
          class="px-8 py-3 gradient-primary text-white font-bold rounded-xl hover:shadow-lg transition-all flex items-center gap-2 group"
          @click="startPathPractice"
        >
          <PlayCircle
            :size="16"
        ...
        <button
          v-else
          class="px-8 py-3 bg-neutral-bg text-neutral-title font-bold rounded-xl hover:bg-neutral-border/50 transition-all flex items-center gap-2"
        >
          <Clock :size="16" />
          练习中...
        </button>
