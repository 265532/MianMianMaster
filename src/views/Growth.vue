...
const selectedQuestionBank = ref<QuestionBankSummary | null>(null);
const currentVideoTime = ref(0);
const courseModules = ref<Record<string, unknown>[]>([]);

// 收藏状态
const favoriteBanks = ref<number[]>(
  localStorage.getItem("favoriteBanks")
    ? JSON.parse(localStorage.getItem("favoriteBanks")!)
    : [],
);
...
const learningResources = [
  {
    ...
    description:
      "本课程从Vue3的基础概念开始，逐步深入到高级特性，包括Composition API、响应式原理、组件通信等核心内容。通过实战项目，让你掌握Vue3的开发技巧和最佳实践。",
    modules: [
  ...
  },
  {
    ...
    description:
      "本书从TypeScript的基础语法开始，逐步深入到高级特性，包括类型系统、泛型、装饰器、高级类型等内容。通过实战案例，让你掌握TypeScript的开发技巧和最佳实践。",
    chapters: [
  ...
  },
  {
    ...
    description:
      "本教程涵盖了前端工程化的各个方面，包括构建工具、代码规范、CI/CD、性能优化等内容。通过实际项目案例，让你掌握前端工程化的最佳实践。",
    modules: [
  ...
  },
  {
    ...
    description:
      "本课程深入解析React Hooks的原理和使用方法，包括useState、useEffect、useContext、useReducer等常用Hooks，以及自定义Hooks的开发技巧。通过实战项目，让你掌握React Hooks的最佳实践。",
    modules: [
      ...
      {
        id: 3,
        title: "useContext 与 useReducer",
        duration: "3小时",
        completed: true,
      },
      {
        id: 4,
        title: "useCallback 与 useMemo",
        duration: "2小时",
        completed: true,
      },
  ...
  },
  {
    ...
    description:
      "本教程从Node.js的基础概念开始，逐步深入到后端开发的各个方面，包括Express框架、数据库操作、API设计、认证授权等内容。通过实战项目，让你掌握Node.js后端开发的技巧和最佳实践。",
    modules: [
  ...
  },
  {
    ...
    description:
      "本书系统介绍了常见的算法和数据结构，包括数组、链表、栈、队列、树、图、排序算法、查找算法等内容。通过大量的实例和练习，让你掌握算法和数据结构的核心概念和应用技巧。",
    chapters: [
  ...
  },
  {
    ...
    description:
      "本课程从前端性能优化的基础概念开始，逐步深入到各种优化策略，包括资源加载、渲染性能、网络优化、缓存策略等内容。通过实际项目案例，让你掌握前端性能优化的核心技巧。",
    modules: [
  ...
  },
  {
    ...
    description:
      "本教程从GraphQL的基础概念开始，逐步深入到GraphQL的各种特性和最佳实践，包括查询、变更、订阅、模式设计等内容。通过实际项目案例，让你掌握GraphQL的开发技巧。",
    modules: [
...
const filteredResources = computed(() => {
  ...
  if (selectedLevel.value !== "all") {
    result = result.filter(
      (resource) => resource.level === selectedLevel.value,
    );
  }
...
const completeModule = (moduleId: number) => {
  ...
  if (module) {
    ...
    if (selectedResource.value) {
      ...
      const completedModules = courseModules.value.filter(
        (m) => m.completed,
      ).length;
      selectedResource.value.progress = Math.round(
        (completedModules / totalModules) * 100,
      );
      // 同时更新原始资源数据
      const originalResource = learningResources.find(
...
const wrongQuestions = [
  {
    ...
    correctAnswer: "A",
    explanation:
      "闭包是指有权访问另一个函数作用域中变量的函数。闭包的应用场景包括：1. 实现私有变量和方法 2. 延长变量的生命周期 3. 实现柯里化 4. 模块化开发。虽然闭包可能导致内存泄漏，但合理使用是安全的。",
  },
  {
    id: 2,
    ...
    correctAnswer: "B",
    explanation:
      "Vue2 使用 Object.defineProperty 实现响应式，只能监听对象的属性变化，无法监听新增属性和删除属性。Vue3 使用 Proxy 实现响应式，可以监听对象的所有操作，包括新增属性、删除属性等，同时性能更好。",
  },
  {
    id: 3,
    ...
    correctAnswer: "D",
    explanation:
      "优化 React 应用性能的方法包括：1. 使用 shouldComponentUpdate 生命周期方法避免不必要的渲染 2. 使用 React.memo 高阶组件缓存组件 3. 使用 useMemo 和 useCallback 钩子缓存计算结果和函数 4. 合理使用 key 属性 5. 避免在渲染过程中创建新函数 6. 使用虚拟列表处理长列表。",
  },
  {
    id: 4,
    ...
    correctAnswer: "A",
    explanation:
      "TCP 三次握手过程：1. 客户端发送 SYN 包请求建立连接 2. 服务器发送 SYN+ACK 包确认接收并请求建立连接 3. 客户端发送 ACK 包确认接收，连接建立。四次挥手过程：1. 客户端发送 FIN 包请求关闭连接 2. 服务器发送 ACK 包确认接收 3. 服务器发送 FIN 包请求关闭连接 4. 客户端发送 ACK 包确认接收，连接关闭。",
  },
  {
    id: 5,
    ...
    correctAnswer: "A",
    explanation:
      "事件循环是 JavaScript 处理异步操作的机制，它会不断循环执行任务队列中的任务。宏任务包括 setTimeout、setInterval、I/O 操作等，微任务包括 Promise.then、process.nextTick 等。微任务的优先级高于宏任务，在每次宏任务执行完成后，会先执行所有微任务，然后再执行下一个宏任务。",
  },
  {
    id: 6,
    question: "如何实现深拷贝？",
    ...
    correctAnswer: "A",
    explanation:
      "JSON.parse(JSON.stringify(obj)) 是一种实现深拷贝的方法，但它有一些限制，比如不能处理函数、正则表达式、循环引用等。Object.assign() 和扩展运算符 {...obj} 只能实现浅拷贝，不能处理嵌套对象。对于复杂对象，需要使用递归或第三方库（如 lodash.cloneDeep）来实现深拷贝。",
  },
  {
    id: 7,
    question: "什么是防抖和节流？",
    ...
    correctAnswer: "A",
    explanation:
      "防抖（debounce）是指在一定时间内多次触发同一事件，只执行最后一次，常用于搜索输入、窗口 resize 等场景。节流（throttle）是指在一定时间内多次触发同一事件，只执行一次，常用于滚动、鼠标移动等场景。",
  },
];
...
const jobQuestionBanks = [
  {
    ...
    difficulty: "高级",
    description:
      "覆盖Java核心概念、多线程、JVM等高频面试题，包含大量实战案例和详细解析",
    progress: 30,
    ...
    questions: [
      {
        ...
        correctAnswer: "C",
        explanation:
          "ConcurrentHashMap是线程安全的，而ArrayList、HashMap和HashSet都不是线程安全的。ConcurrentHashMap通过分段锁机制提高了并发性能。",
        difficulty: "中级",
        tags: ["多线程", "集合"],
      },
      {
        ...
        options: [
          ...
          "B. 堆、栈、程序计数器",
          "C. 堆、栈、方法区、程序计数器",
          "D. 堆、栈、方法区、程序计数器、本地方法栈",
        ],
        correctAnswer: "D",
        explanation:
          "JVM内存结构包括堆、栈、方法区、程序计数器和本地方法栈。其中堆是最大的内存区域，用于存储对象实例。",
        difficulty: "高级",
        tags: ["JVM", "内存管理"],
      },
      {
        ...
        correctAnswer: "B",
        explanation:
          "Java使用自动垃圾回收机制，通过垃圾回收器自动回收不再使用的对象内存，减少内存泄漏的可能性。",
        difficulty: "高级",
        tags: ["JVM", "垃圾回收"],
      },
    ],
  },
  {
    ...
    difficulty: "中级",
    description:
      "专注于Go语言并发特性和最佳实践，包含大量实战案例和性能优化技巧",
    progress: 15,
    ...
    questions: [
      {
        ...
        correctAnswer: "A",
        explanation:
          "Goroutine是Go语言中的轻量级线程，由Go运行时管理，比系统线程更轻量，启动速度更快，内存占用更小。",
        difficulty: "中级",
        tags: ["goroutine", "并发"],
      },
      {
        ...
        correctAnswer: "D",
        explanation:
          "Channel在Go语言中用于goroutine之间的通信和同步，是Go语言实现并发的核心机制。",
        difficulty: "中级",
        tags: ["channel", "并发"],
      },
      {
        ...
        options: [
          "A. 互斥锁只允许一个goroutine访问，读写锁允许多个读操作同时进行",
          "B. 互斥锁允许多个goroutine访问，读写锁只允许一个goroutine访问",
          "C. 互斥锁用于读操作，读写锁用于写操作",
          "D. 没有区别",
        ],
        correctAnswer: "A",
        explanation:
          "互斥锁（Mutex）只允许一个goroutine访问共享资源，而读写锁（RWMutex）允许多个读操作同时进行，但写操作需要独占访问。",
        difficulty: "高级",
        tags: ["并发", "锁"],
      },
    ],
  },
  {
    ...
    difficulty: "高级",
    description:
      "深入Vue3内部实现原理和核心概念，包含响应式系统、编译原理等核心技术",
    progress: 45,
    ...
    questions: [
      {
        ...
        correctAnswer: "B",
        explanation:
          "Vue3的响应式系统基于ES6的Proxy实现，相比Vue2的Object.defineProperty，Proxy可以监听更多的操作，如属性的添加和删除。",
        difficulty: "高级",
        tags: ["响应式", "Proxy"],
      },
      {
        ...
        correctAnswer: "D",
        explanation:
          "Composition API提供了更好的代码组织、更好的类型推断和更好的逻辑复用，使代码更加模块化和可维护。",
        difficulty: "中级",
      ...
      },
      {
        ...
        options: [
          "A. 没有区别",
          "B. Vue3的虚拟DOM使用了Fragment和Teleport",
          "C. Vue3的虚拟DOM性能更差",
          "D. Vue3的虚拟DOM不支持diff算法",
        ],
        correctAnswer: "B",
        explanation:
          "Vue3的虚拟DOM引入了Fragment（片段）和Teleport（ teleport）等新特性，同时对diff算法进行了优化，提高了渲染性能。",
        difficulty: "高级",
        tags: ["虚拟DOM", "渲染"],
      },
    ],
  },
  {
    ...
    difficulty: "中级",
    description:
      "MySQL查询优化、索引设计和性能调优，包含大量实战案例和最佳实践",
    progress: 25,
    ...
    questions: [
      {
        ...
        correctAnswer: "A",
        explanation:
          "索引的主要作用是提高查询速度，但会增加存储开销和降低插入、更新、删除操作的速度。",
        difficulty: "中级",
        tags: ["索引", "性能优化"],
      },
      {
        ...
        options: [
          ...
          "B. SELECT * FROM users WHERE name LIKE %test%",
          "C. SELECT * FROM users WHERE age > 18",
          "D. SELECT * FROM users ORDER BY id",
        ],
        correctAnswer: "B",
        explanation:
          "LIKE %test%会导致全表扫描，因为索引无法被有效利用。而其他选项在有适当索引的情况下可以使用索引。",
        difficulty: "中级",
        tags: ["SQL", "性能优化"],
      },
      {
        ...
        correctAnswer: "D",
        explanation:
          "MySQL的事务隔离级别包括读未提交（READ UNCOMMITTED）、读已提交（READ COMMITTED）、可重复读（REPEATABLE READ）和串行化（SERIALIZABLE）。",
        difficulty: "高级",
        tags: ["事务", "隔离级别"],
      },
    ],
  },
  {
    ...
    difficulty: "中级",
    description:
      "React Hooks的使用技巧和最佳实践，包含大量实战案例和性能优化方法",
    progress: 20,
    ...
    questions: [
      {
        ...
        correctAnswer: "A",
        explanation:
          "useState Hook用于在函数组件中管理状态，返回一个状态变量和一个更新状态的函数。",
        difficulty: "初级",
        tags: ["useState", "Hooks"],
      },
      {
        ...
        correctAnswer: "D",
        explanation:
          "依赖数组控制副作用的执行时机，只有当依赖项发生变化时才会重新执行副作用，这样可以提高性能并避免无限循环。",
        difficulty: "中级",
      ...
      },
      {
        ...
        options: [
          "A. useMemo用于缓存计算结果，useCallback用于缓存函数",
          "B. useMemo用于缓存函数，useCallback用于缓存计算结果",
          "C. 没有区别",
          "D. useMemo用于类组件，useCallback用于函数组件",
        ],
        correctAnswer: "A",
        explanation:
          "useMemo用于缓存计算结果，useCallback用于缓存函数，两者都是为了优化性能，避免不必要的重新计算和函数创建。",
        difficulty: "高级",
    ...
    questions: [
      {
        ...
        correctAnswer: "B",
        explanation:
          "二叉树的前序遍历顺序是根-左-右，中序遍历是左-根-右，后序遍历是左-右-根。",
        difficulty: "中级",
        tags: ["二叉树", "遍历"],
      },
      {
        ...
        correctAnswer: "B",
        explanation:
          "快速排序的平均时间复杂度是O(n log n)，最坏情况是O(n^2)，但通过随机选择 pivot 可以减少最坏情况的发生。",
        difficulty: "高级",
        tags: ["排序", "时间复杂度"],
      },
      {
        ...
        correctAnswer: "B",
        explanation:
          "二分查找是一种对数时间复杂度（O(log n)）的查找算法，只能用于有序数组，通过每次将查找范围减半来快速定位目标元素。",
        difficulty: "中级",
        tags: ["搜索", "二分查找"],
      },
    ],
    ...
    questions: [
      {
        ...
        correctAnswer: "D",
        explanation:
          "Webpack是一个模块打包器，可以将多个模块打包成一个或多个bundle，同时支持代码压缩、转译等功能。",
        difficulty: "中级",
        tags: ["Webpack", "工程化"],
      },
      {
        ...
        options: [
          "A. Vite使用ES模块，Webpack使用CommonJS",
          "B. Vite开发环境更快，Webpack生产环境更稳定",
          "C. Vite只支持Vue，Webpack支持多种框架",
          "D. 没有区别",
        ],
        correctAnswer: "B",
        explanation:
          "Vite在开发环境中使用原生ES模块，启动速度更快；Webpack在生产环境中经过长期优化，更加稳定可靠。",
        difficulty: "中级",
  ...
  },
  {
    ...
    difficulty: "中级",
    description:
      "Node.js后端开发技术和最佳实践，包含Express、MongoDB等核心技术",
    progress: 20,
    ...
    questions: [
      {
        ...
        correctAnswer: "D",
        explanation:
          "Node.js具有单线程、非阻塞I/O和事件驱动的特点，使其适合处理高并发请求。",
        difficulty: "中级",
        tags: ["Node.js", "后端"],
      },
      {
        ...
        correctAnswer: "D",
        explanation:
          "Express是一个基于Node.js的Web应用框架，可以用于构建Web应用、API，以及处理中间件等。",
        difficulty: "中级",
...
const filteredQuestionBanks = computed(() => {
  ...
  if (selectedBankCategory.value !== "all") {
    result = result.filter(
      (bank) => bank.category === selectedBankCategory.value,
    );
  }
  ...
  if (selectedBankDifficulty.value !== "all") {
    result = result.filter(
      (bank) => bank.difficulty === selectedBankDifficulty.value,
    );
  }
...
const startPractice = (bank: QuestionBankSummary) => {
  selectedQuestionBank.value = bank;
...
const submitAnswer = () => {
  ...
  if (selectedAnswer.value) {
    isAnswerCorrect.value =
      selectedAnswer.value === currentQuestion.correctAnswer;
...
const completePractice = () => {
  ...
  correctAnswers.value = answeredQuestions.value.filter(
    (val) => val === true,
  ).length;
  ...
  if (selectedQuestionBank.value) {
    selectedQuestionBank.value.progress = Math.min(
      100,
...
const showToast = (message: string) => {
  ...
  toast.className =
    "fixed top-4 right-4 bg-neutral-title text-white px-4 py-2 rounded-xl shadow-lg z-50 animate-fadeIn";
...
<template>
  <div class="flex flex-col gap-8 max-w-7xl mx-auto">
    ...
    >
      <div class="relative z-10 max-w-2xl">
        ...
        <p class="text-white/80 mb-8 text-lg leading-relaxed">
          强化学习引擎，覆盖错题本与上料存档，让每一次技能进阶都清晰可见。
        </p>
        <div class="flex flex-wrap gap-4">
          <div
            v-for="stat in stats"
            :key="stat.label"
            class="bg-white/10 backdrop-blur-md px-6 py-4 rounded-3xl border border-white/10 flex items-center gap-4"
          >
            <component :is="stat.icon" :size="24" :class="stat.color" />
            <div>
              <p class="text-[10px] text-white/60 uppercase font-bold">
                {{ stat.label }}
              </p>
      ...
      <div class="p-8">
        ...
        <div v-if="activeTab === 'growth'" class="space-y-8">
          ...
          >
            ...
            <!-- 成长轨迹图表 -->
            <div class="mb-8">
              <h3 class="text-sm font-bold text-neutral-title mb-4">
                能力成长趋势
              </h3>
            ...
            <div class="mb-8">
              ...
              <div class="mb-4">
                <h4
                  class="text-xs font-bold text-auxiliary-green mb-2 flex items-center gap-2"
                >
                ...
                <ul class="space-y-2 pl-6">
                  <li
                    ...
                    :key="index"
                    class="text-xs text-neutral-body list-disc"
                  >
                    {{ strength }}
                  </li>
                </ul>
              ...
              <div class="mb-4">
                <h4
                  class="text-xs font-bold text-auxiliary-orange mb-2 flex items-center gap-2"
                >
                  <Target :size="14" />
                  薄弱环节
                </h4>
                <ul class="space-y-2 pl-6">
                  <li
                    v-for="(weakness, index) in analysisData.weaknesses"
                    :key="index"
                    class="text-xs text-neutral-body list-disc"
                  >
                    {{ weakness }}
              ...
              <div class="mb-4">
                <h4
                  class="text-xs font-bold text-primary mb-2 flex items-center gap-2"
                >
                  <Zap :size="14" />
                  学习建议
                </h4>
            ...
            <div class="mt-8 space-y-4">
              <div
                ...
                :key="skill.name"
                class="p-4 bg-neutral-bg rounded-2xl border border-neutral-border/50 hover:bg-white hover:shadow-md transition-all"
              >
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="toggleSubSkills(skill.name)"
                >
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    >
                    <component
                      :is="showSubSkills[skill.name] ? ChevronUp : ChevronDown"
                      :size="16"
                ...
                <!-- 子技能详情 -->
                <div
                  v-if="showSubSkills[skill.name]"
                  class="mt-4 pl-12 space-y-3 animate-fadeIn"
                >
                  <div
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2">
                      <div
                        class="h-1.5 w-24 bg-white rounded-full overflow-hidden"
                      >
                        <div
                          class="h-full bg-primary transition-all duration-1000 ease-out"
        ...
        <div v-if="activeTab === 'resources'" class="space-y-8">
          ...
          >
            <h2
              class="text-xl font-bold text-neutral-title mb-8 flex items-center gap-3"
            >
            ...
            <div class="mb-8 space-y-4">
              ...
              <div class="relative">
                ...
                />
                <div
                  class="absolute left-4 top-1/2 transform -translate-y-1/2 text-neutral-helper"
                >
                  <svg
              ...
              <div class="flex flex-wrap items-center gap-4">
                <div class="flex items-center gap-2">
                  ...
                  <select
                    ...
                    class="bg-neutral-bg border border-neutral-border rounded-xl px-3 py-2 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/30"
                  >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div
                v-for="resource in showAllResources
                  ? filteredResources
                  : filteredResources.slice(0, 4)"
                :key="resource.id"
              ...
              >
                ...
                <div class="flex items-center justify-between mb-4">
                  <div class="flex items-center gap-4">
                    ...
                    <button
                      class="p-2 rounded-full hover:bg-white/50 transition-all"
                    ...
                    >
                      <svg
                        ...
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="lucide lucide-heart text-auxiliary-orange"
                      >
                        <path
                ...
                <div class="mb-4">
                  <h3
                    class="font-bold text-neutral-title mb-2 group-hover:text-primary transition-colors"
                  >
                    {{ resource.title }}
                  </h3>
                  <div class="flex items-center gap-2 mb-3">
                    <span
                      class="text-[10px] px-2 py-1 bg-primary/10 text-primary rounded-full"
                      >{{ resource.type }}</span
                    >
                    <span
                      class="text-[10px] px-2 py-1 bg-neutral-border rounded-full"
                      >{{ resource.level }}</span
                    >
                    <span
                      class="text-[10px] px-2 py-1 bg-neutral-border rounded-full"
                  ...
                  <div class="flex items-center gap-2 mb-3">
                    ...
                    <div class="flex items-center gap-1">
                      <svg
                        ...
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        class="lucide lucide-star text-auxiliary-orange"
                      >
                        <polygon
                          points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"
                        ></polygon>
                      </svg>
                      <span class="text-[10px] font-bold text-neutral-title">{{
                        resource.rating
                      }}</span>
                      <span class="text-[10px] text-neutral-helper"
                ...
                <div class="mb-4">
                  <div
                    class="flex justify-between text-[10px] text-neutral-helper mb-1"
                  >
                    <span>学习进度</span>
                ...
                <div class="flex gap-3">
                  <button
                    class="flex-1 px-4 py-2 bg-neutral-bg text-neutral-title font-bold rounded-xl hover:bg-neutral-border/50 transition-all text-[10px]"
                    @click="viewResourceDetails(resource.id)"
                  >
                    查看详情
                  </button>
                  <button
                    class="flex-1 px-4 py-2 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all text-[10px]"
                    @click="continueLearning(resource.id)"
                  >
                    继续学习
                  </button>
                </div>
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                class="p-4 bg-primary/10 rounded-[20px] border border-primary/20"
              >
          ...
          <!-- 岗位专属题库 -->
          <div
            class="bg-white rounded-[32px] p-6 shadow-sm border border-neutral-border flex flex-col"
          >
            ...
            <div class="space-y-4 mb-6">
              ...
              <div class="relative">
                ...
                />
                <div
                  class="absolute left-3 top-1/2 transform -translate-y-1/2 text-neutral-helper"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
              ...
              <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center gap-2 flex-1 min-w-[140px]">
                  ...
                  >
                  <select
                    v-model="selectedBankCategory"
                    class="bg-neutral-bg border border-neutral-border rounded-xl px-3 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/30 flex-1"
                  >
                    <option value="all">全部分类</option>
                ...
                <div class="flex items-center gap-2 flex-1 min-w-[140px]">
                  ...
                  >
                  <select
                    v-model="selectedBankDifficulty"
                    class="bg-neutral-bg border border-neutral-border rounded-xl px-3 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/30 flex-1"
                  >
                    <option value="all">全部难度</option>
                    <option value="初级">初级</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
              <div
                v-for="bank in showAllBanks
                  ? filteredQuestionBanks
                  : filteredQuestionBanks.slice(0, 4)"
                :key="bank.id"
                class="p-5 bg-neutral-bg rounded-[24px] group hover:bg-white hover:shadow-lg transition-all border border-transparent hover:border-neutral-border cursor-pointer"
              ...
              >
                <div class="flex items-center justify-between mb-3">
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      class="p-1.5 rounded-full hover:bg-white/50 transition-all"
                      @click.stop="addToFavorites(bank.id)"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        ...
                        :class="
                          isBankFavorite(bank.id)
                            ? 'lucide lucide-heart-fill text-auxiliary-orange'
                            : 'lucide lucide-heart text-auxiliary-orange'
                        "
                      >
                        <path
                          d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"
                  ...
                  <span
                    ...
                    >{{ bank.questionCount }}+ 题</span
                  >
                </div>
                <p class="text-[10px] text-neutral-helper mb-3 line-clamp-2">
                  {{ bank.description }}
                </p>
                <div class="flex flex-wrap gap-2 mb-4">
                  ...
                  >
                  <span
                    class="text-xs px-2 py-1 bg-neutral-border rounded-full"
                    >{{ bank.difficulty }}</span
                  >
                  <span
                    v-for="tag in bank.tags.slice(0, 2)"
                    :key="tag"
                    class="text-xs px-2 py-1 bg-neutral-border rounded-full"
                    >{{ tag }}</span
                  >
                ...
                <div class="flex flex-col gap-3">
                  <div class="flex items-center gap-2">
                    <span class="text-[10px] text-neutral-helper w-20"
                      >学习进度</span
                    >
                    <div
                      class="h-1.5 flex-1 bg-white rounded-full overflow-hidden"
                    >
                      <div
                        class="h-full bg-primary transition-all duration-1000 ease-out"
                    ...
                    <span class="text-[10px] font-bold text-primary ml-2">
                      {{ bank.progress }}%</span
                    >
                  </div>
                  <div class="flex gap-3 w-full">
                    <button
                      class="flex-1 px-4 py-1.5 text-[10px] font-bold bg-primary text-white rounded-lg hover:bg-primary-dark transition-all shadow-sm"
                      @click.stop="startPractice(bank)"
                    >
                      开始练习
                    </button>
                    <button
                      class="flex-1 px-4 py-1.5 text-[10px] font-bold bg-white text-primary rounded-lg hover:bg-primary/10 transition-all shadow-sm border border-primary/20"
                      @click="viewQuestionBank(bank)"
                    >
                      查看详情
                    </button>
            ...
            <div
              class="mt-6 p-5 bg-primary/5 rounded-[24px] border border-primary/20 flex items-center gap-3"
            >
              <div
                class="w-12 h-12 rounded-2xl gradient-primary flex items-center justify-center text-white shadow-md"
              >
                <PlayCircle :size="24" />
              </div>
              <div class="flex-1">
                <p class="text-sm font-bold text-primary">个性化推荐</p>
                <p class="text-[10px] text-neutral-body leading-tight">
                  基于您的匹配度推荐专属题库
                </p>
              </div>
            </div>
            ...
            <button
              class="mt-4 w-full py-3 bg-neutral-bg text-neutral-title font-bold rounded-xl hover:bg-neutral-border/50 transition-all text-[10px] flex items-center justify-center gap-2"
        ...
        <div v-if="activeTab === 'engine'" class="space-y-8">
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div
                class="p-8 gradient-yellow-orange rounded-[32px] text-white shadow-lg relative overflow-hidden group cursor-pointer active:scale-95 transition-all"
              ...
              >
                <div class="relative z-10">
                  ...
                  <p class="text-white/80 text-sm mb-6 leading-relaxed">
                    系统自动存档所有历史面试中的薄弱知识点，助您精准突击。
                  </p>
                  <div class="flex items-center justify-between">
                    <button
                      class="px-6 py-3 bg-white text-auxiliary-orange font-bold rounded-xl shadow-md"
                    >
                ...
                <div
                  class="absolute -right-6 -bottom-6 opacity-20 group-hover:scale-110 transition-transform"
                >
                  <RotateCcw :size="140" />
                </div>
              </div>
              <div
                class="p-8 bg-neutral-title rounded-[32px] text-white shadow-lg relative overflow-hidden group cursor-pointer active:scale-95 transition-all"
                @click="showLearningPlans = !showLearningPlans"
              >
                <div class="relative z-10">
                  <h3 class="text-2xl font-black mb-2 text-white">
                    学习计划管理
                  </h3>
                  <p class="text-white/80 text-sm mb-6 leading-relaxed">
                    基于您的薄弱环节，智能生成个性化学习计划，助您系统提升。
                  </p>
                  <div class="flex items-center justify-between">
                    <button
                      class="px-6 py-3 bg-primary text-white font-bold rounded-xl shadow-md"
                    >
            ...
            <div v-if="showWrongQuestions" class="mt-8 space-y-6">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <button
                    class="px-4 py-2 bg-auxiliary-orange/10 text-auxiliary-orange text-xs font-bold rounded-xl hover:bg-auxiliary-orange/20 transition-all flex items-center gap-2"
              ...
              <div class="space-y-4">
                <div
                  ...
                  :key="question.id"
                  class="p-6 bg-neutral-bg rounded-[24px] hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-neutral-border cursor-pointer group"
                >
                  <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                      <h4
                        class="text-sm font-bold text-neutral-title mb-3 group-hover:text-primary transition-colors"
                      >
                        {{ question.question }}
                      </h4>
                      <div class="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        >
                        <span
                          class="text-xs px-3 py-1 bg-neutral-border rounded-full"
                        ...
                        <div
                          ...
                          :class="{
                            'bg-auxiliary-orange/10 border border-auxiliary-orange/30':
                              option.includes(question.userAnswer) &&
                              question.userAnswer !== question.correctAnswer,
                            'bg-auxiliary-green/10 border border-auxiliary-green/30':
                              option.includes(question.correctAnswer),
                          ...
                          <div
                            ...
                            :class="{
                              'bg-auxiliary-orange text-white':
                                option.includes(question.userAnswer) &&
                                question.userAnswer !== question.correctAnswer,
                              'bg-auxiliary-green text-white': option.includes(
                                question.correctAnswer,
                              ),
                              'bg-white border border-neutral-border text-neutral-body':
                                !option.includes(question.userAnswer) &&
                                !option.includes(question.correctAnswer),
                            }"
                          >
                      ...
                      <div class="mb-4">
                        <div class="flex items-center gap-2 mb-2">
                          ...
                          >
                          <span
                            class="text-xs font-bold text-auxiliary-orange"
                            >{{ question.userAnswer }}</span
                          >
                        </div>
                        <div class="flex items-center gap-2 mb-2">
                          <span class="text-xs font-bold text-neutral-title"
                          ...
                          >
                          <span
                            class="text-xs font-bold text-auxiliary-green"
                            >{{ question.correctAnswer }}</span
                          >
                        </div>
                        <div
                          class="mt-3 p-3 bg-primary/5 rounded-xl border border-primary/20"
                        >
                    ...
                    <div class="flex flex-col items-end gap-2">
                      <span class="text-xs font-bold text-auxiliary-orange"
                        >错误 {{ question.times }} 次</span
                      >
                      <span class="text-xs text-neutral-helper"
                        >最后尝试: {{ question.lastAttempt }}</span
                      >
                    </div>
                  </div>
                  <div class="flex items-center gap-3">
                    <button
                      class="flex-1 px-4 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all text-sm"
                      @click="viewWrongQuestion(question)"
                    >
                      查看解析
                    </button>
                    <button
                      class="px-4 py-3 bg-neutral-bg text-neutral-title font-bold rounded-xl hover:bg-neutral-border/50 transition-all text-sm"
                      @click="practiceWrongQuestion(question)"
                    >
                      重新练习
                    </button>
                  </div>
              ...
              <div
                v-if="isReviewing"
                class="p-6 bg-primary/5 rounded-[24px] border border-primary/20"
              >
                <div class="flex items-center justify-between mb-4">
                ...
                <div
                  class="h-2 bg-primary/20 rounded-full overflow-hidden mb-4"
                >
                  <div
              ...
              <div class="space-y-6">
                ...
                >
                  <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                      <h4
                        class="text-sm font-bold text-neutral-title mb-2 group-hover:text-primary transition-colors"
                      >
                        {{ plan.title }}
                      </h4>
                      ...
                      <div class="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        <span
                          class="text-xs px-3 py-1 bg-neutral-border rounded-full"
                          >{{ plan.currentDay }}/{{ plan.totalDays }} 天</span
                        >
                      </div>
                    </div>
                    <div class="flex flex-col items-end gap-2">
                      <div
                        class="h-2 w-32 bg-white rounded-full overflow-hidden"
                      >
                        <div
                          class="h-full bg-primary transition-all duration-1000 ease-out"
                    ...
                    <div class="space-y-2">
                      ...
                      >
                        <input
                          ...
                          @change="completeTask(plan.id, task.id)"
                        />
                        <span class="text-xs text-neutral-body flex-1">{{
                          task.title
                        }}</span>
                        <span class="text-xs text-neutral-helper"
                          >第 {{ task.day }} 天</span
                  ...
                  <div class="flex items-center gap-3">
                    <button
                      class="flex-1 px-4 py-3 bg-neutral-bg text-neutral-title font-bold rounded-xl hover:bg-neutral-border/50 transition-all text-sm"
                      @click="viewLearningPlan(plan)"
                    >
                      查看详情
                    </button>
                    <button
                      v-if="plan.status === '未开始'"
                      class="px-4 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all text-sm"
                      @click="startLearningPlan(plan.id)"
                    >
                      开始学习
                    </button>
                    <button
                      v-else
                      class="px-4 py-3 bg-auxiliary-green text-white font-bold rounded-xl hover:bg-auxiliary-green/80 transition-all text-sm"
                      @click="continueLearningPlan(plan)"
                    >
                      继续学习
                    </button>
                  </div>
        ...
        <div class="p-8">
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div
                class="p-6 bg-primary/10 rounded-2xl border border-primary/20"
              >
                ...
                <div class="space-y-4">
                  <div class="flex justify-between text-sm mb-1">
                    ...
                    <span class="text-primary font-bold"
                      >{{ selectedResource?.progress }}%</span
                    >
                  </div>
                  <div
                    class="h-2 bg-neutral-border rounded-full overflow-hidden"
              ...
              >
                ...
                <div class="space-y-3">
                  <div
                    v-if="selectedResource?.instructor"
                    class="flex items-center gap-3"
                  >
                    <div
                    ...
                    <div>
                      ...
                      <div class="text-xs text-neutral-helper">
                        {{ selectedResource.instructorTitle }}
                      </div>
                    </div>
                  </div>
                  <div
                    v-else-if="selectedResource?.author"
                    class="flex items-center gap-3"
                  >
                    <div
                      class="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold"
                  ...
                  <div
                    v-if="selectedResource?.publishDate"
                    class="text-xs text-neutral-helper"
                  >
            ...
            <div
              class="p-6 bg-neutral-bg rounded-2xl border border-neutral-border"
            >
              <h3 class="font-bold text-neutral-title mb-4">
                {{
                  selectedResource?.type === "电子书" ? "章节列表" : "课程模块"
                }}
              </h3>
              <div class="space-y-3">
                <div
                  v-for="(item, index) in selectedResource?.modules ||
                ...
                >
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        class="px-3 py-1.5 bg-primary text-white text-xs font-bold rounded-lg hover:bg-primary-dark transition-all"
                      >
                        查看
                      </button>
                    </div>
            ...
            <div class="mt-8 flex flex-col sm:flex-row gap-4">
              <button
                ...
                @click="continueLearning(selectedResource?.id)"
              >
                继续学习
                <ChevronRight :size="20" />
              </button>
              <button
                class="flex-1 py-4 bg-white text-primary font-bold rounded-2xl shadow-sm hover:bg-primary/10 transition-all text-[10px] border border-primary/20"
                @click="closeResourceModal"
              >
                关闭
              </button>
    ...
    <!-- 继续学习模态框 -->
    <div
      v-if="showContinueLearningModal"
      class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
      <div
        class="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300"
      >
        ...
        <div class="p-8">
          <div class="space-y-6">
            ...
            >
              <div
                class="aspect-video bg-black/30 rounded-xl flex items-center justify-center"
              >
            ...
            >
              ...
              <div class="mt-3 flex justify-end">
                <button
                  class="px-4 py-2 bg-primary text-white text-sm font-bold rounded-xl hover:bg-primary-dark transition-all"
                >
                  保存笔记
                </button>
              </div>
        ...
        <div class="p-8">
          <div class="space-y-6">
            ...
            <div
              class="p-6 bg-neutral-bg rounded-2xl border border-neutral-border"
            >
              <h3 class="text-lg font-bold text-neutral-title mb-4">
                {{ selectedWrongQuestion?.question }}
              </h3>
              <div class="flex flex-wrap items-center gap-2 mb-4">
                <span
                  class="text-xs px-3 py-1 bg-primary/10 text-primary rounded-full"
                  >{{ selectedWrongQuestion?.category }}</span
                >
                <span
                  class="text-xs px-3 py-1 bg-neutral-border rounded-full"
                  >{{ selectedWrongQuestion?.difficulty }}</span
                >
                <span
                  class="text-xs px-3 py-1 rounded-full"
                  :class="
                    selectedWrongQuestion?.status === '未掌握'
                      ? 'bg-auxiliary-orange/10 text-auxiliary-orange'
                      : 'bg-auxiliary-green/10 text-auxiliary-green'
                  "
                >
                ...
                <span
                  class="text-xs px-3 py-1 bg-auxiliary-orange/10 text-auxiliary-orange rounded-full"
                >
            ...
            >
              ...
              <div class="space-y-3">
                <div
                  ...
                  :class="{
                    'bg-auxiliary-orange/10 border-auxiliary-orange/30':
                      option.includes(selectedWrongQuestion?.userAnswer) &&
                      selectedWrongQuestion?.userAnswer !==
                        selectedWrongQuestion?.correctAnswer,
                    'bg-auxiliary-green/10 border-auxiliary-green/30':
                      option.includes(selectedWrongQuestion?.correctAnswer),
            ...
            <div class="p-6 bg-primary/5 rounded-2xl border border-primary/20">
              ...
              <div class="flex items-center gap-2 mb-4">
                <span class="text-sm font-bold text-neutral-title"
                  >正确答案:</span
                >
                <span class="text-sm font-bold text-auxiliary-green">{{
                  selectedWrongQuestion?.correctAnswer
                }}</span>
              </div>
              <div class="mt-4">
    ...
    <!-- 学习计划详情模态框 -->
    <div
      v-if="showLearningPlanDetailModal"
      class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
      <div
        class="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300"
        ...
        <div class="p-8">
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div
                class="p-6 bg-neutral-bg rounded-2xl border border-neutral-border md:col-span-2"
              >
                <h3 class="text-sm font-bold text-neutral-title mb-3">
                  计划描述
                </h3>
                <p class="text-sm text-neutral-body">
                  {{ selectedLearningPlan?.description }}
                </p>
                <div class="mt-4">
                  <div
                    class="h-2 bg-neutral-border rounded-full overflow-hidden mb-2"
                  >
                    <div
                      class="h-full bg-primary transition-all"
                      :style="{ width: selectedLearningPlan?.progress + '%' }"
                    ></div>
                  </div>
                  <div class="flex justify-between text-xs text-neutral-helper">
                    <span>开始</span>
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                      <input
                        ...
                        class="w-4 h-4 rounded border-neutral-border text-primary focus:ring-primary/30"
                        @change="
                          completeTask(selectedLearningPlan?.id, task.id)
                        "
                      />
                      <button
                        class="px-3 py-1.5 bg-primary text-white text-xs font-bold rounded-lg hover:bg-primary-dark transition-all"
    ...
    <!-- 题库详情模态框 -->
    <div
      v-if="showQuestionBankDetailModal"
      class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
      <div
        class="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300"
      >
        <div class="p-8 border-b border-neutral-border">
          <div class="flex justify-between items-center">
            ...
            <button
              class="w-10 h-10 rounded-full bg-neutral-bg flex items-center justify-center text-neutral-helper hover:bg-primary/10 hover:text-primary transition-all"
              @click="closeQuestionBankDetailModal"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                ...
                <div class="space-y-3">
                  <div class="flex justify-between items-center">
                    <span class="text-xs text-neutral-helper">分类</span>
                    <span class="text-xs font-bold text-neutral-title">{{
                      selectedQuestionBank?.category
                    }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-xs text-neutral-helper">难度</span>
                    <span class="text-xs font-bold text-neutral-title">{{
                      selectedQuestionBank?.difficulty
                    }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-xs text-neutral-helper">题目数量</span>
                    <span class="text-xs font-bold text-primary"
                      >{{ selectedQuestionBank?.questionCount }}+ 题</span
                    >
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-xs text-neutral-helper">学习进度</span>
                    <span class="text-xs font-bold text-primary"
                      >{{ selectedQuestionBank?.progress }}%</span
                    >
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-xs text-neutral-helper">最后更新</span>
                    <span class="text-xs font-bold text-neutral-title">{{
                      selectedQuestionBank?.lastUpdated
                    }}</span>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="tag in selectedQuestionBank?.tags"
                    :key="tag"
                    class="text-xs px-3 py-1 bg-primary/10 text-primary rounded-full"
                    >{{ tag }}</span
                  >
              ...
              <div
                class="p-6 bg-auxiliary-orange/10 rounded-2xl border border-auxiliary-orange/20"
              >
                <h3 class="text-sm font-bold text-auxiliary-orange mb-3">
                  操作
                </h3>
                <button
                  class="w-full px-4 py-3 bg-auxiliary-orange text-white font-bold rounded-xl hover:bg-auxiliary-orange/80 transition-all text-sm mb-3 shadow-sm"
                  @click="startPractice(selectedQuestionBank)"
                >
                  开始练习
                </button>
                <button
                  class="w-full px-4 py-3 bg-white text-auxiliary-orange font-bold rounded-xl hover:bg-auxiliary-orange/10 transition-all text-sm border border-auxiliary-orange/20 shadow-sm"
                  @click="addToFavorites(selectedQuestionBank?.id)"
                >
                  {{
                    isBankFavorite(selectedQuestionBank?.id)
                      ? "取消收藏"
                      : "添加收藏"
                  }}
                </button>
            ...
            >
              ...
              <div class="space-y-2">
                ...
                <div class="h-2 bg-neutral-border rounded-full overflow-hidden">
                  <div
                    class="h-full bg-primary transition-all duration-1000 ease-out"
                    :style="{ width: selectedQuestionBank?.progress + '%' }"
                  ></div>
                </div>
                <div class="flex justify-between text-xs text-neutral-helper">
                  <span>开始</span>
            ...
            <div
              class="p-6 bg-neutral-bg rounded-2xl border border-neutral-border"
            >
              <h3 class="text-lg font-bold text-neutral-title mb-3">
                题库描述
              </h3>
              <p class="text-sm text-neutral-body leading-relaxed">
            ...
            <div>
              ...
              <div class="space-y-4">
                <div
                  v-for="(
                    question, index
                  ) in selectedQuestionBank?.questions.slice(0, 3)"
                  :key="question.id"
                  class="p-5 bg-neutral-bg rounded-2xl border border-neutral-border hover:shadow-md transition-all"
                >
                  <div class="flex items-center gap-2 mb-3">
                    <div
                      class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm"
                  ...
                  <div class="space-y-2 mb-4 pl-10">
                    <div
                      ...
                      :key="optIndex"
                      class="flex items-center gap-3 p-2 rounded-xl bg-white border border-neutral-border hover:border-primary/30 transition-all"
                    >
                      <div
                        class="w-4 h-4 rounded-full flex items-center justify-center text-xs font-bold bg-white border border-neutral-border text-neutral-body"
                      >
                        {{ option.charAt(0) }}
                      </div>
                      <span class="text-xs text-neutral-body">{{
                  ...
                  <div class="flex items-center gap-3 pl-10">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <span
                        ...
                        :key="tagIndex"
                        class="text-xs px-2 py-1 bg-primary/10 text-primary rounded-full ml-1"
                        >{{ tag }}</span
                      >
                    </div>
                  </div>
                ...
                <div
                  v-if="selectedQuestionBank?.questions.length > 3"
                  class="text-center"
                >
                  <button
                    class="px-4 py-2 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all text-sm shadow-sm"
                  >
                    查看全部题目
                  </button>
                </div>
            ...
            <div class="p-6 bg-primary/5 rounded-2xl border border-primary/20">
              ...
              <ul class="space-y-2 pl-6">
                <li class="text-xs text-neutral-body list-disc">
                  覆盖{{ selectedQuestionBank?.category }}领域核心知识点
                </li>
                <li class="text-xs text-neutral-body list-disc">
                  包含{{ selectedQuestionBank?.questionCount }}+道精选题目
                </li>
                <li class="text-xs text-neutral-body list-disc">
                  适合{{ selectedQuestionBank?.difficulty }}难度级别的学习者
                </li>
                <li class="text-xs text-neutral-body list-disc">
                  提供详细的题目解析和答案
                </li>
    ...
    <!-- 题库练习模态框 -->
    <div
      v-if="showQuestionPracticeModal"
      class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
      <div
        class="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300"
      >
        <div class="p-8 border-b border-neutral-border">
          <div class="flex justify-between items-center">
            ...
            <button
              class="w-10 h-10 rounded-full bg-neutral-bg flex items-center justify-center text-neutral-helper hover:bg-primary/10 hover:text-primary transition-all"
              @click="closeQuestionPracticeModal"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
          ...
          <div class="mt-6 flex flex-wrap gap-2">
            <button
              ...
              :class="{
                ...
                'bg-neutral-bg text-neutral-title hover:bg-primary/10':
                  index > currentQuestionIndex,
              }"
        ...
        <div class="p-8">
            ...
            <div class="space-y-6">
              ...
              >
                ...
                <div class="space-y-3 mb-6 pl-12">
                  <div
                    v-for="(option, index) in practiceQuestions[
                      currentQuestionIndex
                    ].options"
                    :key="index"
                    class="flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all transform hover:scale-102"
                    :class="{
                      'bg-auxiliary-orange/10 border border-auxiliary-orange/30 animate-shake':
                        selectedAnswer === option.charAt(0) &&
                        isAnswerCorrect === false,
                      'bg-auxiliary-green/10 border border-auxiliary-green/30 animate-pulse':
                        selectedAnswer === option.charAt(0) &&
                      ...
                      'bg-white border border-neutral-border hover:border-primary/30':
                        selectedAnswer !== option.charAt(0) &&
                        isAnswerCorrect === null,
                    }"
                    @click="
                      !isAnswerCorrect && (selectedAnswer = option.charAt(0))
                    "
                  >
                    <div
                      class="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold"
                ...
                <div
                  ...
                  class="p-5 bg-primary/5 rounded-xl border border-primary/20 animate-fadeIn"
                >
                  <div class="flex items-center gap-2 mb-3">
                    <CheckCircle2
                    ...
                    <svg
                      ...
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      class="lucide lucide-x-circle text-auxiliary-orange"
                    >
              ...
              <div class="flex items-center justify-between">
                <button
                  ...
                  class="px-6 py-3 bg-neutral-bg text-neutral-title font-bold rounded-xl hover:bg-neutral-border/50 transition-all text-sm shadow-sm"
                  :class="
                ...
                <div v-if="!isAnswerCorrect" class="flex-1 max-w-md mx-4">
                  <button
                    ...
                    class="w-full px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all text-sm shadow-sm"
                    :class="
                ...
                <button
                  :disabled="
                    ...
                    !isAnswerCorrect
                  "
                  class="px-6 py-3 bg-auxiliary-green text-white font-bold rounded-xl hover:bg-auxiliary-green/80 transition-all text-sm shadow-sm"
                  :class="
                    currentQuestionIndex === practiceQuestions.length - 1 &&
                    !isAnswerCorrect
                      ? 'opacity-50 cursor-not-allowed'
          ...
          <div v-else-if="practiceCompleted" class="text-center py-12">
            <div
              class="w-24 h-24 rounded-full bg-auxiliary-green/20 flex items-center justify-center mx-auto mb-6"
            >
              <CheckCircle2 :size="48" class="text-auxiliary-green" />
            </div>
            <h3 class="text-2xl font-black text-neutral-title mb-4">
              练习完成！
            </h3>
            <p class="text-sm text-neutral-body mb-8">
              恭喜你完成了{{ selectedQuestionBank?.title }}的练习
            </p>
            <div class="grid grid-cols-3 gap-6 mb-8">
              <div
                class="p-4 bg-primary/10 rounded-2xl border border-primary/20"
              >
              ...
              <div
                class="p-4 bg-auxiliary-green/10 rounded-2xl border border-auxiliary-green/20"
              >
                <p class="text-xs text-neutral-helper mb-1">正确题数</p>
                <p class="text-2xl font-black text-auxiliary-green">
                  {{ correctAnswers }}
                </p>
              </div>
              <div
                class="p-4 bg-auxiliary-orange/10 rounded-2xl border border-auxiliary-orange/20"
              >
                <p class="text-xs text-neutral-helper mb-1">正确率</p>
                <p class="text-2xl font-black text-auxiliary-orange">
                  {{ finalAccuracy }}%
                </p>
              </div>
            </div>
            <div class="flex flex-col sm:flex-row gap-4">
              <button
                class="flex-1 py-4 bg-primary text-white font-bold rounded-2xl shadow-lg hover:shadow-primary/30 transition-all"
                @click="closeQuestionPracticeModal"
              >
                关闭
              </button>
              <button
                class="flex-1 py-4 bg-white text-primary font-bold rounded-2xl shadow-sm hover:bg-primary/10 transition-all border border-primary/20"
