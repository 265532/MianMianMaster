...
const gameLevels = [
  ...
  },
  {
    ...
    objectives: ['完成12个面试问题', '展示系统设计能力', '获得资深工程师面试认证']
  },
  {
    id: 4, 
    name: '关卡 04', 
...
// 当前关卡数据
const currentLevel = ref(gameLevels.find(level => level.id === levelId.value) || gameLevels[0])
...
const difficultyColor = computed(() => {
  switch (currentLevel.value?.难度) {
    case '简单': return 'bg-auxiliary-green/10 text-auxiliary-green'
    case '中等': return 'bg-primary/10 text-primary'
    case '困难': return 'bg-auxiliary-orange/10 text-auxiliary-orange'
    case '专家': return 'bg-auxiliary-purple/10 text-auxiliary-purple'
    case '大师': return 'bg-auxiliary-red/10 text-auxiliary-red'
    default: return 'bg-neutral-border/50 text-neutral-helper'
  }
})
    ...
    <div class="bg-white rounded-[32px] p-8 shadow-sm border border-neutral-border">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
        <div class="flex items-center gap-4">
          <div class="w-16 h-16 rounded-2xl bg-white flex items-center justify-center text-primary shadow-sm" :class="currentLevel?.background">
            <span class="text-3xl">{{ currentLevel?.icon }}</span>
          </div>
          <div>
            <div class="flex items-center gap-2 mb-2">
              <span class="text-sm font-bold text-primary">{{ currentLevel?.name }}</span>
              <span v-if="currentLevel?.status === '已解锁'" class="px-3 py-1 bg-auxiliary-green/10 text-auxiliary-green text-xs font-bold rounded-full">已解锁</span>
              <span v-else class="px-3 py-1 bg-neutral-border/50 text-neutral-helper text-xs font-bold rounded-full">未解锁</span>
            </div>
            <h2 class="text-2xl font-bold text-neutral-title mb-1">{{ currentLevel?.title }}</h2>
            <p class="text-sm text-neutral-helper">{{ currentLevel?.description }}</p>
          </div>
        </div>
        <div class="flex items-center gap-3 px-4 py-2 rounded-xl" :class="difficultyColor">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="p-4 bg-neutral-bg rounded-[20px]">
          ...
          <p class="text-lg font-bold text-neutral-title">{{ currentLevel?.时间限制 }} 分钟</p>
        </div>
        <div class="p-4 bg-neutral-bg rounded-[20px]">
          <div class="flex items-center gap-2 mb-2">
          ...
          <p class="text-lg font-bold text-neutral-title">{{ currentLevel?.问题数量 }} 题</p>
        </div>
        <div class="p-4 bg-neutral-bg rounded-[20px]">
          <div class="flex items-center gap-2 mb-2">
          ...
          <p class="text-lg font-bold text-neutral-title">{{ currentLevel?.successRate }}</p>
        </div>
      </div>
      ...
      <div class="mb-8">
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="(skill, index) in currentLevel?.skills" :key="index" class="px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-full">
            {{ skill }}
          </span>
        </div>
      </div>
      ...
      <div class="mb-8">
        ...
        <div class="space-y-2">
          <div v-for="(objective, index) in currentLevel?.objectives" :key="index" class="flex items-center gap-3 p-3 bg-neutral-bg rounded-[16px]">
      ...
      <!-- Progress -->
      <div class="mb-8">
        <div class="flex justify-between text-sm font-bold text-neutral-title mb-2">
          <span>关卡进度</span>
          <span>{{ currentLevel?.progress }}%</span>
        </div>
        <div class="h-2 bg-neutral-border rounded-full overflow-hidden">
          <div class="h-full bg-primary transition-all duration-1000" :style="{width: currentLevel?.progress + '%'}"></div>
        </div>
      </div>
      ...
      <div class="flex gap-4">
        ...
        <button 
          ...
          :class="currentLevel?.status !== '已解锁' ? 'bg-neutral-border text-neutral-helper cursor-not-allowed' : ''"
        >
          <Zap :size="20" />
