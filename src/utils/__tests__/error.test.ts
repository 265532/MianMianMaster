...
describe("error utils", () => {
  ...
  describe("formatErrorMessage", () => {
    ...
    it("should prioritize response.data.message over error.message", () => {
      const error = {
        response: { data: { message: "response msg" } },
        message: "error msg",
      };
  ...
  describe("isTimeoutError", () => {
    ...
    it("should detect timeout in message", () => {
      expect(isTimeoutError({ message: "timeout of 15000ms exceeded" })).toBe(
        true,
      );
    });
    ...
    it("should not detect non-timeout error", () => {
      expect(
        isTimeoutError({ code: "ERR_NETWORK", message: "Network Error" }),
      ).toBe(false);
    });
  });
  ...
  describe("handleApiError", () => {
    it("should not throw for client errors", () => {
      expect(() =>
        handleApiError({ code: 400, message: "bad request" }),
      ).not.toThrow();
    });

    it("should not throw for server errors", () => {
      expect(() =>
        handleApiError({ code: 500, message: "internal error" }),
      ).not.toThrow();
    });
  });
});
