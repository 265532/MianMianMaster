...
vi.mock('../http', () => ({
  default: {
    ...
    post: vi.fn((url, data, config) => Promise.resolve({ data: {}, url, body: data, ...config })),
    put: vi.fn((url, data, config) => Promise.resolve({ data: {}, url, body: data, ...config })),
    delete: vi.fn((url, config) => Promise.resolve({ data: {}, url, ...config }))
  }
}))
...
describe('request utils', () => {
  ...
  describe('del', () => {
    it('should call http.delete with url and params', async () => {
      ...
      expect(http.delete).toHaveBeenCalledWith('/users/1', { params: { reason: 'test' } })
    })
  })
