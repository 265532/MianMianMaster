...
export const comprehensiveAssessmentQuestions: AssessmentQuestion[] = [
  // 岗位技能测评 - 前端开发
  {
    ...
    weight: 5,
    description:
      "前端框架是现代前端开发的核心工具，熟悉多种框架会增加您的竞争力。",
  },
  {
    id: 2,
    category: "岗位技能",
  ...
  },
  {
    ...
    weight: 4,
    description:
      "TypeScript已成为现代前端开发的标准工具，掌握它会提高代码质量。",
  },
  {
    id: 4,
    category: "岗位技能",
  ...
  },
  {
    ...
    weight: 4,
    description:
      "数据库是后端系统的核心组件，熟悉多种数据库系统会提高您的设计能力。",
  },
  {
    id: 7,
    category: "岗位技能",
    ...
    weight: 4,
    description:
      "API设计是后端开发的重要技能，良好的API设计会提高系统的可维护性。",
  },
  ...
  },
  {
    ...
    weight: 3,
    description:
      "自我认知是职业发展的基础，准确的自我评估有助于制定合理的发展计划。",
  },
...
export const getQuestionsByType = (
  ...
  switch (type) {
    case "jobSkill":
      return comprehensiveAssessmentQuestions.filter(
        (q) => q.category === "岗位技能",
      );
    case "psychological":
      return comprehensiveAssessmentQuestions.filter(
        (q) => q.category === "职业心理" || q.category === "性格测评",
      );
    case "technical":
      return comprehensiveAssessmentQuestions.filter(
        (q) => q.category === "综合技术",
...
export const calculateAssessmentResult = (
  ...
  const totalPossibleScore = questions.reduce((total, q) => {
    if (q.type === "single") {
      const maxScore = Math.max(
        ...q.options!.map((opt) => ("score" in opt ? opt.score || 0 : 0)),
      );
      return total + maxScore * q.weight;
    } else if (q.type === "multiple") {
      const maxScore = q.options!.reduce(
        (sum, opt) => sum + ("score" in opt ? opt.score || 1 : 1),
        0,
      );
      return total + maxScore * q.weight;
  ...
  answers.forEach((answer, index) => {
      ...
      maxQuestionScore =
        Math.max(
          ...question.options!.map((opt) =>
            "score" in opt ? opt.score || 0 : 0,
          ),
        ) * question.weight;
    } else if (question.type === "multiple") {
      const maxOptionScore = question.options!.reduce(
        (sum, opt) => sum + ("score" in opt ? opt.score || 1 : 1),
        0,
      );
    ...
    categoryMaxScores[question.category] =
      (categoryMaxScores[question.category] || 0) + maxQuestionScore;
    if (question.subcategory) {
      subcategoryMaxScores[question.subcategory] =
        (subcategoryMaxScores[question.subcategory] || 0) + maxQuestionScore;
    }
    ...
    categoryScores[question.category] =
      (categoryScores[question.category] || 0) + questionScore;
    if (question.subcategory) {
      subcategoryScores[question.subcategory] =
        (subcategoryScores[question.subcategory] || 0) + questionScore;
    }
  });
  ...
  Object.keys(categoryScores).forEach((category) => {
    detailedScores[category] = Math.round(
      ((categoryScores[category] || 0) / (categoryMaxScores[category] || 1)) *
        100,
    );
  });
  ...
  Object.keys(subcategoryScores).forEach((subcategory) => {
    subcategoryDetailedScores[subcategory] = Math.round(
      ((subcategoryScores[subcategory] || 0) /
        (subcategoryMaxScores[subcategory] || 1)) *
        100,
    );
  });
  ...
  // 分析子类别优势和劣势
  Object.entries(subcategoryDetailedScores).forEach(
  ...
  switch (type) {
    case "jobSkill":
      generateJobSkillRecommendations(
        result,
        scorePercentage,
        subcategoryDetailedScores,
      );
      break;
    case "psychological":
...
// 生成岗位技能测评推荐
const generateJobSkillRecommendations = (
...
// 生成职业心理测评推荐
const generatePsychologicalRecommendations = (
  result: AssessmentResult,
  scorePercentage: number,
  mbtiType: string,
) => {
...
// 生成学习计划测评推荐
const generateLearningPlanRecommendations = (
  result: AssessmentResult,
