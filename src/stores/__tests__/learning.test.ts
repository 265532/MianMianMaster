...
describe('useLearningStore integration', () => {
  ...
  describe('fetchCollections', () => {
    it('should fetch collections and update state', async () => {
      ...
      mockedGetCollections.mockResolvedValue({
        ...
        message: 'success',
        data: [
          { id: 1, title: '高频算法50题', question_count: 50, category: '算法', difficulty: 'medium' },
          { id: 2, title: 'Vue3核心知识', question_count: 30, category: '前端', difficulty: 'easy' }
        ]
      })
  ...
  describe('fetchWrongQuestions', () => {
    it('should fetch wrong questions and update state', async () => {
      ...
      mockedGetWrongQuestions.mockResolvedValue({
        ...
        message: 'success',
        data: [
          { id: 1, question: 'Vue3组件通信方式？', user_answer: 'props', correct_answer: 'props/events/provide/pinia', category: '前端', status: 'unreviewed' },
          { id: 2, question: 'React Hooks规则？', user_answer: '不确定', correct_answer: '只在顶层调用', category: '前端', status: 'unreviewed' }
        ]
      })
  ...
  describe('addToCollection', () => {
    it('should add to collection successfully', async () => {
      ...
      expect(mockedAddToCollection).toHaveBeenCalledWith({ question_bank_id: 1 })
    })
  })
  ...
  describe('markWrongQuestionMastered', () => {
    it('should mark wrong question as mastered', async () => {
      const { learningApi } = await import('@/api/modules/learning.api')
      const mockedMarkMastered = vi.mocked(learningApi.markWrongQuestionMastered)
