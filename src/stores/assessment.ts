...
export const useAssessmentStore = defineStore("assessment", () => {
  ...
  async function fetchAssessments(): Promise<void> {
    ...
    try {
      ...
      error.value =
        err?.response?.data?.message || err?.message || "获取测评列表失败";
  ...
  async function createAssessment(data: {
    ...
    try {
      ...
      error.value =
        err?.response?.data?.message || err?.message || "创建测评失败";
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  async function submitAssessment(
    id: number,
    data: { type?: string; answers?: Record<string, any> },
    ...
    try {
      ...
      error.value =
        err?.response?.data?.message || err?.message || "提交测评失败";
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  async function fetchResult(assessmentId: number): Promise<void> {
    ...
    try {
      ...
      error.value =
        err?.response?.data?.message || err?.message || "获取测评结果失败";
