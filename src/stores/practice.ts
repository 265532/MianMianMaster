...
export const usePracticeStore = defineStore('practice', () => {
  ...
  async function fetchBanks(): Promise<void> {
    ...
    try {
      ...
      banks.value = response.data.map((course: any) => ({
        id: course.id,
        title: course.title,
        description: course.description || '',
        category: course.category || '通用',
        questionCount: course.question_count || 0,
        estimatedTime: course.estimated_time || '30分钟',
        passRate: course.pass_rate || 70,
  ...
  function nextQuestion(): void {
    if (currentBank.value && currentQuestionIndex.value < currentBank.value.questions.length - 1) {
