...
<template>
  <div class="min-h-screen flex bg-neutral-bg">
    ...
    >
      ...
      <div class="flex-1 py-6 px-3 space-y-2 overflow-y-auto overflow-x-hidden">
        <div 
          ...
          :class="route.path === item.path ? 'bg-primary/10 text-primary font-bold shadow-sm' : 'text-neutral-body hover:bg-neutral-bg'"
        >
          <component :is="item.icon" :size="20" :class="route.path === item.path ? 'text-primary' : 'text-neutral-helper group-hover:text-primary'" />
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      >
        <div class="flex items-center gap-4">
          <button @click="toggleSidebar" class="hidden md:flex p-2 hover:bg-neutral-bg rounded-lg text-neutral-helper transition-all-300">
            <Menu v-if="!isSidebarOpen" :size="20" />
            <X v-else :size="20" />
          </button>
          <button @click="toggleMobileMenu" class="md:hidden p-2 hover:bg-neutral-bg rounded-lg text-neutral-helper">
            <Menu :size="20" />
          </button>
          
          <div class="relative hidden sm:block">
            <Search class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-helper" :size="16" />
            <input 
              type="text" 
    ...
    <Transition name="fade">
      <div v-if="isMobileMenuOpen" class="fixed inset-0 bg-black/50 z-[60] md:hidden" @click="toggleMobileMenu"></div>
    </Transition>
    
    <!-- Mobile Sidebar -->
    <Transition name="slide">
      <div v-if="isMobileMenuOpen" class="fixed left-0 top-0 bottom-0 w-72 bg-white z-[70] md:hidden flex flex-col shadow-2xl">
        <div class="h-16 flex items-center justify-between px-6 border-b border-neutral-border">
        ...
        <div class="flex-1 py-6 px-4 space-y-2">
          <div 
            ...
            :class="route.path === item.path ? 'bg-primary/10 text-primary font-bold' : 'text-neutral-body hover:bg-neutral-bg'"
          >
